...
const challengeCards: Array<{
  ...
  { key: "environment", title: "Cuide do ambiente", eyebrow: "Responsabilidade", description: "Escolha atitudes que reduzem desperdícios e protegem o meio ambiente.", icon: Leaf, tone: "green", stat: "4 perguntas" },
];
...
function App() {
  ...
  const startChallenge = (challenge: ChallengeKey, playerForRound: Player = player) => {
    ...
    const questions = shuffle(pool.length >= 8 ? pool : questionBank.filter((item) => item.difficulty === playerForRound.difficulty)).slice(0, 8).map(shuffleQuestion);
...
function PlayPage(props: {
  ...
  const { player, activeChallenge, startChallenge, gameQuestions, currentQuestion, questionIndex, selectedAnswer, feedback, submitAnswer, nextQuestion, score, earnedThisAnswer, streak, lives, timeLeft, result, foundErrors, chooseError, kanbanCards, selectedKanban, setSelectedKanban, moveKanban, go } = props;
