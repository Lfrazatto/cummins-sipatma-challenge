...
const challengeCards: Array<{
  ...
  { key: "quiz", title: "Quiz de Segurança", eyebrow: "Conhecimento rápido", description: "Aprenda a reconhecer riscos, cuidar de si e proteger quem trabalha com você.", icon: ShieldCheck, tone: "cyan", stat: "44 perguntas" },
...
const questionBank: Question[] = [
  ...
  question(25, "Segurança", "safety", "Médio", "O que uma conversa de segurança eficaz deve fazer?", ["Culpar quem errou", "Ser rápida e sem perguntas", "Focar somente na meta", "Abrir espaço para riscos, dúvidas e controles"], 3, "Uma boa conversa permite que as pessoas relatem riscos e confirmem como trabalhar de forma segura."),
  question(26, "Segurança", "safety", "Médio", "Durante uma movimentação com empilhadeira, pedestres devem:", ["Passar atrás do equipamento se houver espaço", "Manter distância, respeitar rotas e contato visual quando necessário", "Correr para atravessar antes", "Ficar atrás da carga para se proteger"], 1, "Equipamentos móveis exigem segregação, atenção às rotas e comunicação segura entre pedestres e operadores."),
  question(27, "Segurança", "safety", "Difícil", "Qual é a melhor resposta a um desvio recorrente no processo?", ["Aceitar como parte da rotina", "Corrigir apenas quando houver auditoria", "Investigar causas, fortalecer controles e acompanhar a eficácia", "Trocar a pessoa responsável"], 2, "Desvios recorrentes pedem análise de causa e melhoria do sistema, não apenas correção pontual."),
  question(28, "Segurança", "safety", "Difícil", "Em um trabalho a quente, o controle essencial inclui:", ["Permissão aplicável, remoção de combustíveis e controle contra incêndio", "Somente abrir uma janela", "Acelerar a tarefa", "Trabalhar sem comunicar para não interromper"], 0, "Trabalhos a quente exigem avaliação, permissão, controle de fontes combustíveis e prontidão para emergência."),
  question(29, "Segurança", "safety", "Difícil", "Por que controles administrativos não devem ser a única barreira quando há risco grave?", ["Porque são sempre mais caros", "Porque dependem do comportamento e podem falhar sozinhos", "Porque não precisam ser documentados", "Porque substituem proteções físicas"], 1, "A hierarquia de controles prioriza eliminar, substituir e proteger antes de depender apenas de instruções."),
  question(30, "Segurança", "safety", "Difícil", "Uma análise de risco deve ser revisada quando:", ["Apenas no fim do ano", "Somente depois de um acidente", "Quando houver mudança de equipamento, material, método ou condição", "Nunca, se já foi assinada"], 2, "Mudanças podem introduzir novos perigos; a análise deve acompanhar a realidade da tarefa."),
  question(31, "Segurança", "safety", "Difícil", "Qual atitude demonstra autoridade para parar o trabalho?", ["Interromper com respeito, explicar o risco e buscar avaliação antes de continuar", "Parar apenas quando alguém da liderança pedir", "Esconder a preocupação para não atrasar", "Continuar e registrar depois"], 0, "Qualquer pessoa deve poder interromper uma condição insegura e pedir ajuda sem medo de retaliação."),
  question(32, "Segurança", "safety", "Difícil", "Quando um incidente acontece, a prioridade inicial é:", ["Definir culpados", "Preservar a produção", "Comunicar apenas no fim do turno", "Cuidar das pessoas, controlar o perigo e acionar a resposta adequada"], 3, "A resposta inicial deve proteger pessoas, controlar a fonte do perigo e iniciar a comunicação prevista."),
  ...
  question(69, "5S", "fiveS", "Médio", "Ao propor retirar um item de uma área, é importante:", ["Descartar imediatamente", "Confirmar uso, dono, condição e destino correto", "Esconder em outra área", "Esperar que desapareça"], 1, "A decisão evita descarte indevido e transfere o problema para o local correto."),
  question(70, "5S", "fiveS", "Difícil", "Qual resultado indica um 5S maduro?", ["A área fica perfeita apenas no dia da auditoria", "As pessoas identificam desvios e corrigem o padrão no cotidiano", "Não existem registros de problemas", "Tudo depende de uma pessoa"], 1, "Maturidade aparece quando o cuidado está incorporado à rotina e gera aprendizado contínuo."),
